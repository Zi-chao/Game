...
export const TicTacToeGame = ({ onBack }: TicTacToeGameProps) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-950 to-slate-900 flex flex-col items-center justify-center p-4 relative">
      ...
      <ClearCacheButton storageKeys={['tictactoe_best']} onCleared={() => window.location.reload()} />
        ...
        <div className="bg-slate-800 rounded-lg px-4 py-2 border border-slate-700">
          ...
          <div className="text-xl font-bold flex items-center gap-2">
            {state.mode === 'pve' ? (
              ...
              <span className="text-rose-400">○ 玩家2</span>
            )}
          </div>
        </div>
        <div className="bg-slate-800 rounded-lg px-4 py-2 border border-slate-700">
            ...
            {state.mode === 'pve' ? (
              state.winner !== aiSide ? (
                <>
                  ...
                  <div className="text-4xl font-bold text-blue-400 mb-2 animate-pulse">你赢了！</div>
                </>
              ) : (
                <>
                  <div className="text-5xl mb-2">💔</div>
                  <div className="text-4xl font-bold text-rose-400 mb-2 animate-pulse">电脑获胜</div>
                </>
              )
            ) : state.winner === 1 ? (
              <>
                <div className="text-5xl mb-2 animate-bounce">🏆</div>
                <div className="text-4xl font-bold text-blue-400 mb-2 animate-pulse">玩家1获胜！</div>
              </>
            ) : (
              <>
                <div className="text-5xl mb-2">🏆</div>
                <div className="text-4xl font-bold text-rose-400 mb-2 animate-pulse">玩家2获胜！</div>
              </>
            )}
            <button
              onClick={() => reset()}
