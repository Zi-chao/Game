...
export const PongGame = ({ onBack }: PongGameProps) => {
  const { state, requiredScore, start, reset, togglePause, gameWidth, gameHeight } = usePong();
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-950 to-slate-900 flex flex-col items-center justify-center p-4 relative">
      ...
      <div className="flex gap-4 mb-3">
        <div className="bg-emerald-900/40 border-2 border-emerald-500 rounded-xl px-5 py-2">
          <div className="text-emerald-300 text-xs">🟢 玩家1 (底部 A/D)</div>
        ...
        <div className="bg-slate-800 rounded-xl px-4 py-2 border border-slate-700 flex flex-col justify-center">
          ...
          <div className="text-lg font-bold text-amber-400 font-mono">{requiredScore} 球</div>
        </div>
      </div>
      ...
      <div className="relative">
        <canvas
          ...
          width={gameWidth}
          height={gameHeight}
          className="rounded-lg shadow-2xl border-4 border-purple-700"
        />
        ...
        {(state.winner !== null || !state.isPlaying) && (
          <div className="absolute inset-0 bg-black/70 backdrop-blur-sm rounded-lg flex flex-col items-center justify-center">
            ...
            {!state.isPlaying && state.winner === null && (
              <>
                <div className="text-3xl font-bold text-purple-400 mb-4">重力小球</div>
                <div className="text-sm text-slate-300 text-center mb-2 max-w-md">
                  <span className="text-emerald-300">🟢 玩家1</span> 在底部，A/D 左右移动<br />
                  <span className="text-blue-300">🔵 玩家2</span> 在顶部，方向键移动<br />
                  反弹球，让对方接不住！先得 {requiredScore} 分获胜
                </div>
              </>
            )}
            {state.isPaused && (
            ...
            <button
              ...
              className="mt-4 px-8 py-3 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-xl transition-all transform hover:scale-105 shadow-lg"
            >
